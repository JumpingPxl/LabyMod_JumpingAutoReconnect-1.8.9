...
public class ModGuiDisconnected extends GuiScreen {
	...
	public ModGuiDisconnected(JumpingAddon jumpingAddon, GuiDisconnected guiDisconnected)
		...
		message = (IChatComponent) ReflectionHelper.findField(GuiDisconnected.class, MESSAGE_MAPPINGS)
				.get(guiDisconnected);
	...
	public void initGui() {
		buttonList.clear();
		multilineMessage = fontRendererObj.listFormattedStringToWidth(message.getFormattedText(),
				width - 50);
		reasonHeight = multilineMessage.size() * fontRendererObj.FONT_HEIGHT;
		buttonList.add(new GuiButton(0, width / 2 - 10,
				height / 2 + reasonHeight / 2 + fontRendererObj.FONT_HEIGHT, 125, 20,
				I18n.format("gui.toMenu")));
		reconnectButton = new GuiButton(1, width / 2 - 115,
				height / 2 + reasonHeight / 2 + fontRendererObj.FONT_HEIGHT, 100, 20,
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawCenteredString(fontRendererObj, reason, width / 2,
				height / 2 - reasonHeight / 2 - fontRendererObj.FONT_HEIGHT * 2, 11184810);
		int i = height / 2 - reasonHeight / 2;
		if (multilineMessage != null) {
			for (Object object : multilineMessage) {
				drawCenteredString(fontRendererObj, String.valueOf(object), width / 2, i, 16777215);
